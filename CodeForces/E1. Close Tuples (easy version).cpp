...
void solve()
{
    ...
    for(ll i=0;i<n;i++)
    {
        ll ub=upper_bound(a.begin(),a.end(),a[i]+2)-a.begin();

        ll midLen=((ub-1)-i)-1;
        //cout<<a[i]<<" "<<midLen<<endl;
        if(midLen>0)
        {
            ans+=(midLen*(midLen+1))/2;
        }
    }
    cout<<ans<<endl;
    ...
    {
        ll p=*it;
        ll ppp=((cnt[p])*(cnt[p]-1)*(cnt[p]-2))/6;
        ll ppp1=(((cnt[p])*(cnt[p]-1))/2)*(cnt[p+1]);
        ll pp1p1=(cnt[p])*(((cnt[p+1])*(cnt[p+1]-1))/2);
        ll pp1p2=(cnt[p])*(cnt[p+1])*(cnt[p+2]);
        ll pp2p2=(cnt[p])*(((cnt[p+2])*(cnt[p+2]-1))/2);
        ll ppp2=(((cnt[p])*(cnt[p]-1))/2)*(cnt[p+2]);
        ans+=(ppp+ppp1+pp1p1+pp1p2+pp2p2+ppp2);
    }
    cout<<ans<<endl;
    */
