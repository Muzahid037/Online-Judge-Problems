...
void solve()
{
    ...
    {
        cin >> a[i];
        if (firstPosOfColor[a[i]]==0)
        {
            firstPosOfColor[a[i]] = i+1;
        }
    }
    ...
    {
        cin >> tq[i];
        cout << firstPosOfColor[tq[i]] << " ";

        for (int j = 1; j <= 50; j++)
        {
            if (firstPosOfColor[tq[i]] > firstPosOfColor[j])
            {
                firstPosOfColor[j]++;
                //firstPosOfColor[j] = (firstPosOfColor[j] % (n - 1));
            }
        }
        firstPosOfColor[tq[i]] = 1;
    }
    cout << endl;
}
int main()
