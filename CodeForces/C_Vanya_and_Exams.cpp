...
void solve()
{
    ll n, r, avg, a, b, currGradeSum = 0;
    cin >> n >> r >> avg;
    pair<ll, ll> x[n];
    for (ll i = 0; i < n; i++)
    {
        cin >> x[i].second >> x[i].first;
        currGradeSum += x[i].second;
    }
    if ((currGradeSum / n) >= avg)
    {
        cout << "0" << endl;
        return;
    }
    ...
    for (ll i = 0; i < n; i++)
    {
        ll addPoss = r - x[i].second;
        ll addNeed=(avg*n)-currGradeSum;
        cost += ((min(addNeed,addPoss))*x[i].first);
        currGradeSum += addPoss;
        if ((currGradeSum / n)>=avg)
            {
                cout << cost << endl;
                return;
