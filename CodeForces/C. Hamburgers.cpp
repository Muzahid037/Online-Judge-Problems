...
int main()
{
    ...
    while(t--)
    {
        ll bRatio=0,sRatio=0,cRatio=0,nb,ns,nc,pb,ps,pc,r,ans=0;
        string ham;
        cin>>ham;
        for(ll i=0; i<ham.size(); i++)
        {
            if(ham[i]=='B') bRatio++;
            else if(ham[i]=='S') sRatio++;
            else cRatio++;
        }
        //cout<<"bRatio:"<<bRatio<<" sRatio:"<<sRatio<<" cRatio:"<<cRatio<<endl;
        ...
        while(low<=high)
        {
            ll mid=(low+high)/2;
            //cout<<mid<<endl;
            ll mx=max(0LL,mid*bRatio-nb)*pb+max(0LL,mid*sRatio-ns)*ps+max(0LL,mid*cRatio-nc)*pc;
            if(mx<=r)
            {
                ans=mid;
                low=mid+1;
